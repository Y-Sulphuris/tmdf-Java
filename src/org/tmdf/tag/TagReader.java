...
public final class TagReader {
	...
	public NamedTag nextTag() {
		...
		Tag<?> tag = readPayload(type);
		return new NamedTag(name,tag);
	}

	private Tag<?> readPayload(byte type) {
		switch (type) {
			case 1:
			...
			case 7:
				return BooleanTag.of(readBoolean());
			case 8:
			...
			case 16:
				...
				return darray;
			case 17:
				BooleanArrayTag boarray = new BooleanArrayTag(readInt());
				for (int i = 0; i <  boarray.length(); i++) {
					boarray.set(i,readBoolean());
				}
				return boarray;
			case 18:
				TagArray tarray = new TagArray(readInt());
